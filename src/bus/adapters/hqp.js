...
class HQPAdapter {
  ...
  getZones(opts = {}) {
    ...
    // HQPlayer represents a single "zone" (the pipeline)
    const zone_id = `hqp:${this.instanceName}`;

    return [{
      zone_id,
      display_name: `HQPlayer ${this.instanceName}`,
      source: 'hqp',
  ...
  getNowPlaying(zone_id) {
    ...
    // Return a minimal now playing state
