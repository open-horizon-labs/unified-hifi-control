...
use crate::adapters::roon_browse::RoonBrowseAdapter;
...
pub struct AppState {
    ...
    pub roon_browse: Arc<RoonBrowseAdapter>,
...
impl AppState {
    ...
    pub fn new(
        ...
        roon_browse: Arc<RoonBrowseAdapter>,
...
pub async fn roon_browse_handler(
    ...
    // Browse to the level
    let browse_result = match state.roon_browse.browse(opts).await {
        Ok(result) => result,
        Err(e) => {
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let items = if let Some(ref list) = browse_result.list {
        if list.count > 0 {
            ...
            match state.roon_browse.load(load_opts).await {
...
pub async fn roon_browse_status_handler(State(state): State<AppState>) -> impl IntoResponse {
    let connected = state.roon_browse.is_connected().await;
