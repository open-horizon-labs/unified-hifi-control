...
final class UHCBonjourDiscovery: NSObject, NetServiceBrowserDelegate, NetServiceDelegate {
    ...
    func stop() {
        ...
        browser.stop(); services.forEach { $0.stop() }; services.removeAll()
    }
    ...
    func netServiceDidResolveAddress(_ sender: NetService) {
        ...
        if let host = sender.hostName?.trimmingCharacters(in: CharacterSet(charactersIn: ".")),
           !host.isEmpty, sender.port > 0, let url = URL(string: "http://\(host):\(sender.port)") {
            finish(url); return
        }
        ...
        if let raw = txt["base"].flatMap({ String(data: $0, encoding: .utf8) }),
           let url = URL(string: raw), url.host != nil { finish(url) }
    }
    ...
    private func finish(_ url: URL) { finished = true; stop(); onBaseURL?(url) }
}
