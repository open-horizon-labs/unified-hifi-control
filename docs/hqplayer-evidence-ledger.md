...
| L2 | Embedded 6.0.2 / engine 6.0.4 | 2026-08-02 | idle | Direct browser-form and native-protocol qualification against isolated named profiles on the user's licensed host: non-identical profile load, native `ConfigurationGet` verification, rollback, cleanup, and normal-service restoration. No content was playing. Root-service backup hazard was also directly observed and the temporary copy removed. |
...
| HQP-C-065 | A named profile is loaded by replaying the authenticated browser form at `POST /config/profile/load`: fresh hidden inputs plus URL-encoded `profile`, with `Origin` and `Referer`. The HTTP response is dispatch evidence only; native `ConfigurationGet` is the outcome authority. A non-identical profile and rollback both passed live. UHC must not use `/backup/settings.zip`: on root-run Embedded that handler was directly observed starting a copy of `/`, including system and media paths. | E0-uhc-live | direct browser-form/native qualification · direct · Embedded 6.0.2 / engine 6.0.4 · 2026-08-02 · idle | test:src/adapters/hqplayer.rs::profile_load_replays_the_browser_form_contract_after_digest_challenge · test:tier2_live_persistent_profile_verification_when_opted_in · test:tests/hqplayer_operation_lease.rs::profile_form_and_native_refresh_stay_on_one_endpoint | settled | — |
...
| HQP-C-044 | The daemon's own **profile save/load/restore** routes are unsafe as a durable preset store: loading a profile restarts the daemon and may leave `/backup/settings.zip` empty until a further restart, and a named active profile may have no `hqplayerd.xml` at all | E1-upstream-verified | UHC-SALVAGE reports via #330 · read-via-report · hqplayerd 6.0.4 (Opal) · 2026-07-29 · idle | test:tier2_live_persistent_profile_verification_when_opted_in | retired | — |
...
The inherited claim incorrectly grouped profile selection with archive backup/restore. HQP-C-065
settles the distinction live: browser-form profile selection works and is verifiable through native
`ConfigurationGet`; archive backup/restore is unnecessary and must not be used by UHC. The stronger
reason is operational, not theoretical: on root-run Embedded, requesting `/backup/settings.zip`
started copying the filesystem root. The temporary copy was removed without touching its sources.
